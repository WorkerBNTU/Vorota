...
- `/sitemap.xml` и `/robots.txt` — с бэкенда; у статических URL в sitemap есть `lastmod`.
- `SITE_URL` (по умолчанию `https://vorota-rb.by`) — **обязателен** на проде.
- Старый домен (`vorotarb.by`) **не** редиректит на этот сайт и наоборот: два независимых сайта.
- **Dynamic rendering** — см. [раздел ниже](#dynamic-rendering-prerendering-для-роботов).
...
- [ ] Обновить SEO-тексты в БД: `python manage.py seed_data --refresh-seo` (или правки в админке «Главная» → SEO).
- [ ] Прогнать `docker compose --profile prerender run --rm prerender` после деплоя контента.
- [ ] **Яндекс.Вебмастер**: добавить сайт, sitemap, регион — **Беларусь** (не только Минск).
- [ ] **Google Search Console**: сайт + sitemap.
- [ ] ID Метрики / GA в админке.
- [ ] HTTPS.
- Полный чеклист: [docs/runbook.md](docs/runbook.md#чеклист-первого-деплоя).
...
- nginx (`frontend/nginx.conf`) по `User-Agent` определяет ботов
  ...
  снапшот; обычные пользователи как и раньше получают SPA. Если снапшота
