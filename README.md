...
- Файрвол (`ufw`): открыть только 22 (SSH), 80 и 443. Порт 5432/6379
  ...
  это уже правильно по умолчанию. Порт 8000 (backend) в текущем конфиге
  открыт наружу для удобства локальной разработки — на боевом сервере
  замените `"8000:8000"` на `"127.0.0.1:8000:8000"` (или уберите совсем):
  весь реальный трафик `/api/` и так идёт через nginx контейнера `frontend`
  на 80/443 (см. `frontend/nginx.conf`), прямой доступ к Django никому,
  кроме вас через SSH-туннель, не нужен.
...
отдельный хостинг изображений**, храните их прямо на сервере в `media/`
(так уже и работает: Docker volume `media_data`, раздаётся тем же nginx,
что и сам сайт). Причины:
- 50–100 МБ — это меньше, чем один пакет JS-библиотек в `node_modules`;
  места он не займёт, даже на минимальном тарифе.
- nginx уже настроен отдавать статику из `media/` с кэш-заголовками
  (`frontend/nginx.conf`) — по скорости отдачи разницы с внешним
  хостингом для такого объёма не будет, а задержка на сторонний домен,
  наоборот, только добавится.
...
**не читают** её: в проде картинки лежат в volume `media_data` →
`backend/media/` внутри контейнера, отдаются по URL `/media/...`.
...
- nginx (`frontend/nginx.conf`) по `User-Agent` определяет ботов
  ...
  конкретного пути снапшота ещё нет — бот получает обычный `index.html`
  (deg­radation без ошибок, а не 404).
